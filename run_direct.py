...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader)):
            ...
            # torch.Size([12, 96, 3]) torch.Size([12, 144, 2]) torch.Size([12, 96, 5]) torch.Size([12, 144, 5])
            # decoder input
        ...
        if accelerator.is_local_main_process:
            accelerator.print(
                "Epoch: {0} | Train Loss: {1:.7f} Vali Loss: {2:.7f} MAE Loss: {4:.7f}".format(
                    epoch + 1, train_loss, vali_loss, vali_mae_loss))
